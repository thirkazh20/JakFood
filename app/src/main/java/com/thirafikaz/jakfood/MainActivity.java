...
import com.thirafikaz.jakfood.Adapter.FoodAdapter;
import com.thirafikaz.jakfood.Model.DataKategoriItem;
...
public class MainActivity extends SessionManager implements FoodAdapter.OnFoodClick, SwipeRefreshLayout.OnRefreshListener{
    ...
    Button btnUploadImageFood, btnUpload, btnCancel,getBtnUpload, btnUpdate, btnDelete, btnUpdateImages;
    ...
    private void fetchDataMakananan(String kategoriMakanan) {
        ...
        ConfigRetrofit.getInstance().getDataMakanan(idUser, kategoriMakanan).enqueue(new Callback<ResponseMakanan>() {
            ...
            public void onResponse(Call<ResponseMakanan> call, Response<ResponseMakanan> response) {
                if (response.isSuccessful()) {
                    ...
                    adapter.setOnClickListener(MainActivity.this::onItemClick);
                }
            }
    ...
    private void fetchInsetData(String kategoriMakanan) {
        try {
...
//                    .setNotificationConfig(new UploadNotificationConfig())
    ...
    private void fetchDataKategori(Spinner spInsert) {
        ConfigRetrofit.getInstance().getkategorimakanan().enqueue(new Callback<ResponseKategoriItem>() {
            @Override
            public void onResponse(Call<ResponseKategoriItem> call, Response<ResponseKategoriItem> response) {
                if (response.isSuccessful() && response.body().getDataKategori() != null) {
                    ...
                    for (int index = 0; index < dataKategori.size(); index++){
                        ...
                        namaKategori[index] = dataKategori.get(index).getNamaKategori();
                    }

                    ArrayAdapter adapter = new ArrayAdapter(MainActivity.this, android.R.layout.simple_spinner_item, namaKategori);
            ...
            @Override
            public void onFailure(Call<ResponseKategoriItem> call, Throwable t) {
    ...
    private void showFileChooser() {
        ...
        startActivityForResult(Intent.createChooser(intentGalery, "Select Pitcures"), REQ_FILE_CODE);
    }
    ...
    private void showFileChooser(int reqFileChoose) {
        ...
        intentGalery.setType("image/*");
        intentGalery.setAction(Intent.ACTION_GET_CONTENT);
        startActivityForResult(Intent.createChooser(intentGalery, "Select Pitcures"), reqFileChoose);
    }
...
//                    .setNotificationConfig(new UploadNotificationConfig())
    ...
    private void deleteMakanan() {
        ConfigRetrofit.getInstance().deleteData(idMakanan).enqueue(new Callback<com.thirafikaz.jakfood.Model.Response>() {
            @Override
